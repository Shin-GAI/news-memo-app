...
// Use Google AI Studio if GEMINI_API_KEY is set, otherwise fall back to Forge API
const resolveApiUrl = () => {
  if (ENV.geminiApiKey) {
...
const assertApiKey = () => {
  if (!resolveApiKey()) {
    throw new Error("No AI API key configured. Set GEMINI_API_KEY (Google AI Studio) or BUILT_IN_FORGE_API_KEY.");
  }
};
